...
# bit error case
# python3 run_fashion_binarized_fi.py --batch-size=256 --epochs=1 --lr=0.001 --step-size=25 --test-error 1 --dry-run 
...
class BNN_FMNIST(nn.Module):
    def __init__(self):
        ...
        self.conv1 = QuantizedConv2d(1, 64, kernel_size=3, padding=1, stride=1, quantization=binarizepm1, error_model=None, quantize_train=q_train, quantize_eval=q_eval)
        self.bn1 = nn.BatchNorm2d(64)
        self.qact1 = QuantizedActivation(quantization=binarizepm1, quantize_train=q_train, quantize_eval=q_eval)

        self.conv2 = QuantizedConv2d(64, 64, kernel_size=3, padding=1, stride=1, quantization=binarizepm1, error_model=None, quantize_train=q_train, quantize_eval=q_eval)
        self.bn2 = nn.BatchNorm2d(64)
        self.qact2 = QuantizedActivation(quantization=binarizepm1, quantize_train=q_train, quantize_eval=q_eval)

        self.fc1 = QuantizedLinear(7*7*64, 2048, quantization=binarizepm1, error_model=SymmetricBitErrorsBinarizedPM1(binarizePM1FI.binarizeFI, 0.1), quantize_train=q_train, quantize_eval=q_eval, snn_sim = snn_sim)
        self.bn3 = nn.BatchNorm1d(2048)
        self.qact3 = QuantizedActivation(quantization=binarizepm1, quantize_train=q_train, quantize_eval=q_eval)

        self.fc2 = QuantizedLinear(2048, 10, quantization=binarizepm1, error_model=SymmetricBitErrorsBinarizedPM1(binarizePM1FI.binarizeFI, 0.1), quantize_train=q_train, quantize_eval=q_eval, snn_sim = snn_sim)
